//注意事项-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------
//第1行到第77行代码为新建头文件 "head.h",在89行进行了导入，使用方法为新建头文件"head.h",里面存入第1行到第77行的代码
//之后新建源文件,导入81-449行代码，即可开始运行
//对于音乐可以自己进行替换，在149行 MyPlayMusic 函数，如若报错，将音乐部分函数注释即可，不影响程序运行
#define DEF_SNAKE_LONG 20 //蛇的最大长度
